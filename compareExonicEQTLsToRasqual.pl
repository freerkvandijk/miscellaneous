...
#Loop through exonic eQTL file
#Original file used: exon_level_eQTLs_Top_effects.exonicSNPsOnly_10E-8.txt
#open(EQTL, "< chr1to5.exonic.eQTLs.txt") or die("Unable to open eQTL file"); #Read file
...
my $totalThres=0;
my $counter=0;
for (my $i=0; $i<=$#eQTLfile; $i++){ #Change $i to 1 when using file containing header
    my $line = $eQTLfile[$i];
    chomp $line;
    my @array = split("\t", $line);
        ...
        if (defined $grep && $grep ne "") {
            ...
            while (my $line = <CHUNK>){
                ...
                if ($loc eq $cloc) {
                    ...
                    print OUTPUT "$loc\t$refAll\t$altAll\t";
                    ...
                    print OUTPUT "$assessedAll\t$zScore\t$cloc\t$crefAll\t$caltAll\t$AF\t$chiSq\t$effSize\t\t$updatedZscore\t$zScoreDirection\t$correctedEffSize\t$effSizeDirection\t$NoFSNPs\t$NoSNPs\t$SqCor\t";
                }
            ...
            print OUTPUT "$rank\t$normRank\t$rounded\n";
            }
        }else{
            `echo $line >> tmp.txt`;
        }
...
close(OUTPUT);
my $perc = ($conc/$total)*100;
my $percThres = ($concThres/$totalThres)*100;
print "\nDONE\n\nCounts: $conc\t$disconc\t$total\t$perc\n";
print "$concThres\t$disconcThres\t$totalThres\t$percThres\n";
