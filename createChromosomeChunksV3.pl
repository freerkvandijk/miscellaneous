...
#/groups/umcg-gcc/prm02/projects/pdeelen/expressedGenes.txt
#perl createChromosomeChunksV3.pl -gtfFile /apps/data/ftp.ensembl.org/pub/release-75/gtf/homo_sapiens/Homo_sapiens.GRCh37.75.gtf -dictFile /apps/data/ftp.broadinstitute.org/bundle/2.8/b37/human_g1k_v37.dict -geneFile expressedGenesBlood20170213.txt -outputFile output.txt
...
while (my $line = <GTF>) {
    chomp $line;
    #Read line, extract chr,Êstart pos, end pos, Ensembl Gene Symbol from gene features only
    if ($line !~ m/^#.+/gs) { #Check if line starts with # symbole, if not it's not a header line
        ...
        $chr =~ s/Y/24/g;
        my $feature = $array[2];
        my $start = ($array[3]-15000);
        my $end = ($array[4]+15000);
...
for (my $i=1; $i <= 22; $i++){ #Iterate over all chromosomes, 1 to 22
    ...
    for (my $k=0; $k <= $lastIdx; $k++){ #Iterate over final arrays, printing the new regions
        ...
        my $currEnd = $Ends[$k];
        #my $prevStart = $Starts[$k];
        my $prevStart = ($Ends[$k-1]+1);
        my $prevEnd = $Ends[$k];
        my $nextStart = $Starts[$k];
        my $nextEnd = $Ends[$k];
...
Usage: createChromosomeChunksV3.pl
